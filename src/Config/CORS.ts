...
export default function CORS(app: any) {
    ...
    if (typeof (tempM) === 'string') {
        if (tempM === 'false') { console.log('Cors Not permited!'); }
        else if (tempM === '*') { console.log('Cors Allowed: *'); app.use(cors()); }
        else if (tempM.includes('[') && tempM.includes(']')) {
            try {
